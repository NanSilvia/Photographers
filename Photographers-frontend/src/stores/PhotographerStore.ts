...
const setupBroadcastListeners = (set: any, _get: any) => {
    ...
    broadcastApi.on<Photographer>('photographerAdded', (newPhotographer) => {
        set((state: PhotographerStoreState) => {
            ...
            if(state.photographers.find((photog) => photog.id === newPhotographer.id) !== undefined) {
                return {};
            }
            return {
                photographers: [...state.photographers, {
                    ...
                    death: newPhotographer.death ? new Date(newPhotographer.death) : null,
                }]
            };
        });
    });
    ...
    broadcastApi.on<Photographer>('photographerUpdated', (updatedPhotographer) => {
        set((state: PhotographerStoreState) => ({
            photographers: state.photographers.map((photog) =>
                photog.id === updatedPhotographer.id ? {
                    ...updatedPhotographer,
                    birth: new Date(updatedPhotographer.birth),
                    death: updatedPhotographer.death ? new Date(updatedPhotographer.death) : null,
                } : photog
            ),
        }));
    });
    ...
    broadcastApi.on<number>('photographerDeleted', (deletedId) => {
        set((state: PhotographerStoreState) => ({
            photographers: state.photographers.filter((photog) => photog.id !== deletedId),
        }));
    });
};
